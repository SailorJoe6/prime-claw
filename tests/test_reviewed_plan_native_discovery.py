...
def provider(path,records,location,socket_path=None):
 transport = "" if socket_path is None else f"process.env.PRIME_AGENT_INTERNAL_DAEMON_SUPERVISOR_SOCKET={json.dumps(str(socket_path))};const expectedSocket={json.dumps(str(socket_path))};"
 guard = "" if socket_path is None else 'if(process.env.PRIME_AGENT_INTERNAL_DAEMON_SUPERVISOR_SOCKET!==expectedSocket)throw Error("fake transport not effective");'
...
def test_installed_discovery_real_implement_spec_activation_resume_and_absence(tmp_path, request):
 ...
 rows=[json.loads(x) for x in records.read_text().splitlines()];assert all(row["transportIsFake"] is True for row in rows);assert rows[0]["kernel"]==1 and rows[-1]["package"]==1;types=[command.get("type") for command in daemon.commands];assert "create" in types and types.count("prompt")==2 and "ack_result" in types
...
def test_installed_completed_generation_allows_later_cycle_and_old_replay(tmp_path, request):
    ...
    provider_source=r'''process.env.PRIME_AGENT_INTERNAL_DAEMON_SUPERVISOR_SOCKET=__SOCKET__;
import{appendFileSync}from"node:fs";import{createAssistantMessageEventStream}from"@earendil-works/pi-ai";
...
'''
    probe.write_text(provider_source.replace("__SOCKET__",json.dumps(str(sock))).replace("__RECORDS__",json.dumps(str(records))).replace("__LOCATION__",json.dumps(location)))
    project=tmp_path/"project";project.mkdir();git(project,"init","-q");git(project,"config","user.email","poc@example.invalid");git(project,"config","user.name","POC")
    for name in ["implement-spec","handoff","execute"]:
        target=project/f".ralph/skills/{name}/SKILL.md";target.parent.mkdir(parents=True,exist_ok=True);target.write_text(f"---\nname: {name}\n---\n{name}")
    ...
    sessions=tmp_path/"sessions";sessions.mkdir();env2={**os.environ,"PRIME_AGENT_CODING_AGENT_DIR":str(agent),"PROBE_MODE":"later"}
    cp=run([PRIME,"--mode","text","--offline","--session-dir",str(sessions),"--no-skills","--no-prompt-templates","--no-context-files","--cwd",str(project),"--provider","poc","--model","m","-p",f"/implement-spec {location}"],env2);assert cp.returncode==0,cp.stdout+cp.stderr
    beta=json.loads((state/f"{slug}.json").read_text());assert beta["bootstrapAdmission"]=="delivered"
    rows=[json.loads(x) for x in records.read_text().splitlines()];assert len(rows)>=3 and rows[1]["package"]==1 and rows[-1]["package"]==1
...
def test_installed_session_start_recovers_completing_without_model_tool(tmp_path, request):
    assert PRIME
    agent=tmp_path/"agent";env={**os.environ,"PRIME_AGENT_PLUGIN_ROOT":str(agent)};assert run([str(APPLY)],env).returncode==0
    sock=Path(f"/tmp/pc-daemon-{os.getpid()}-{time.time_ns()}.sock");daemon=FakeDaemon(sock);records=tmp_path/"records.jsonl";probe=agent/"extensions/probe-provider.ts";provider(probe,records,".ralph/plans/future/unused",sock)
    request.addfinalizer(lambda: (daemon.close(), sock.unlink(missing_ok=True)))
    ...
    env2={**os.environ,"PRIME_AGENT_CODING_AGENT_DIR":str(agent),"PROBE_MODE":"plain"};sessions=tmp_path/"sessions";sessions.mkdir()
    seeded=run([PRIME,"--mode","text","--offline","--session-dir",str(sessions),"--no-skills","--no-prompt-templates","--no-context-files","--no-extensions","--cwd",str(project),"-e",str(setup),"-e",str(probe),"--provider","poc","--model","m","-p","seed recovery"],env2);assert seeded.returncode==0,seeded.stdout+seeded.stderr
    owner_file=next(sessions.glob("*.jsonl"));cp=run([PRIME,"--mode","text","--offline","--session-dir",str(sessions),"--no-skills","--no-prompt-templates","--no-context-files","--cwd",str(project),"--provider","poc","--model","m","--resume",str(owner_file),"-p","ordinary after recovery"],env2);assert cp.returncode==0,cp.stdout+cp.stderr
    receipt=json.loads((state/"recovery.finalization.json").read_text());assert receipt["state"]=="completed";assert not (state/"recovery.json").exists();rows=[json.loads(x) for x in records.read_text().splitlines()];assert len(rows)==2 and all(row["transportIsFake"] is True and row["package"]==0 for row in rows);assert any(c.get("type")=="list" for c in daemon.commands)
    daemon.close();sock.unlink(missing_ok=True);git(project,"branch","-D","episode/recovery")
