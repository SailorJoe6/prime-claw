...
| 8a | **sandbox-egress-reset** | Sandbox `git ls-remote` gets `Recv failure: Connection reset by peer`, and even an allowed HTTPS canary returns HTTP 000 / curl 56 while the host can reach GitHub. | Check `zetup vpn status` first; a `not-connected` result requires operator VPN authentication before repeating a bounded sandbox check. VPN reconnection did not restore sandbox egress in the 2026-09-25 incident; if Git and the canary still reset, coordinate safe host/OpenShell data-path repair while preserving the sandbox databases. This pattern does not prove VPN is the only cause. Do not touch Git provider credentials or use a host push as a workaround; do not claim the local commit is remote. See `docs/evidence/phase3a-slice1-transport-20260925.json`. |
