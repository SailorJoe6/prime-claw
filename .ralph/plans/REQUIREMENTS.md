...
| R-U1-6 | GATE | Credential injection is proven end-to-end via the OpenShell provider/placeholder model: (a) the agent environment holds opaque placeholders only; (b) a real credentialed service call succeeds through the in-sandbox proxy; (c) no credential material exists on sandbox disk; (d) fail-closed behavior on unresolvable placeholders is demonstrated. |
