...
- **R-CO-32 (GATE) — Handoff chain.** Approved work advances through native `/handoff`, one best-effort compaction attempt, and exactly one canonical `execute` continuation unless continuation itself is infeasible. Execute delivery does not depend on successful compaction or `session_compact`.
...
- **R-CO-48 (GATE) — Verified durable primer.** Before handoff, the conversation verifies that authoritative durable artifacts name the correct next phase or same-gate revision, finding IDs, acceptance conditions, and confirmed operator intent. A compaction summary is optional evidence and never gates execute.
- **R-CO-49 (GATE) — Auto-compaction race safety.** Auto-compaction cannot authorize progression. The controller prevents, pauses, supersedes, or recovers any compaction that races with gate adjudication or omits late findings, then verifies authoritative durable state before handoff; it does not wait for a replacement summary before mandatory continuation.
